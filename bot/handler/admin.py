...
from bot.handler.start import admins
...
async def back_to_menu(msg: Message):
    if msg.from_user.id in admins:
        await msg.answer(html_decoration.bold('Bosh menu'), parse_mode='HTML', reply_markup=menu_button(admin=True))
...
async def settings_group_handler(call: CallbackQuery, state: FSMContext):
    if call.from_user.id in admins:
        ...
        if data[1] == 'access':
            ...
            await call.answer(html_decoration.bold(f'Id: {data[-1]} o\'quvchi o\'chdi'), parse_mode='HTML',
                              show_alert=True)
            ...
            await call.message.delete()
            await call.message.edit_text(html_decoration.bold('Guruh'), parse_mode='HTML',
                                         reply_markup=await inl_group())
